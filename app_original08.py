# ==========================================================
# Rent vs. Buy Decision Support Framework (Styled Version)
# ==========================================================
# This version consolidates UI styling and cleans duplicate helpers:
# - Unified section_header and thick_divider
# - Consolidated CSS into one consistent theme
# - Standardized buy/rent accents across metrics, tables, and charts
# - Zebra-striping for tables, highlighted totals
# - Consistent chart legends, gridlines, and colors
...
# --- UI helper functions (injected to prevent NameError and standardize visuals) ---
def section_header(title: str, subtitle: str = None):
    """Top-level section header with optional subtitle."""
    import streamlit as st
    st.header(title)
    if subtitle:
        st.caption(subtitle)
...
def thick_divider():
    """A visually thicker divider between major sections."""
    import streamlit as st
    st.markdown('<hr style="height:3px;border:none;background:#e5e7eb;margin:1.2rem 0;" />', unsafe_allow_html=True)

def style_plot(fig, legend_y: float = 1.08):
    """Standardize Plotly styling across the app."""
    fig.update_layout(
...
try:
    px.defaults.color_discrete_sequence = ["#6A1B9A", "#1E88E5", "#6366F1", "#7F7F7F", "#A0A0A0"]
except Exception:
    pass
...
# --- GLOBAL METRIC STYLES (compact but larger) ---
st.markdown(
    """
    <style>
...
st.markdown("""
<style>
/* Default subheaders (outside containers = section headers) */
h3, h4 {
  margin: .6rem 0 .4rem !important;
  font-weight: 600 !important;
  padding: .3rem .65rem !important;
  border-radius: 4px !important;
  background: #f3f4f6 !important;   /* darker gray than before */
  border-left: 3px solid var(--accent) !important;
  color: var(--ink) !important;
}

/* Subheaders inside generic containers */
div[data-testid="stContainer"] h3,
div[data-testid="stContainer"] h4 {
  background: #f9fafb !important;   /* subtle light gray */
  border-left: 2px solid #d1d5db !important;  /* soft muted accent */
...
/* Subheaders in Buy containers */
.card-tint.buy h3,
.card-tint.buy h4 {
  background: #f9f5fc !important;   /* very light purple tint */
  border-left: 3px solid #6A1B9A !important;
  color: #6A1B9A !important;
}

/* Subheaders in Rent containers */
.card-tint.rent h3,
.card-tint.rent h4 {
  background: #f0f6fc !important;   /* very light blue tint */
...
def apply_theme():
    ...
    div[data-testid="stContainer"][style*="border: 1px solid"] {
    ...
    .card-tint.buy::before,
    .card-tint.rent::before {
      content:"";
      position:absolute;
...
st.markdown(
    """
    <div style='display:flex; gap:1.5rem; margin: .5rem 0 1rem 0;'>
        <span style='display:flex; align-items:center; gap:.4rem'>
            <div style='width:14px; height:14px; border-radius:50%; background:#6A1B9A'></div>
            <span>Buy</span>
        </span>
        <span style='display:flex; align-items:center; gap:.4rem'>
            <div style='width:14px; height:14px; border-radius:50%; background:#1E88E5'></div>
            <span>Rent</span>
        </span>
        <span style='display:flex; align-items:center; gap:.4rem'>
    ...
    with st.container(border=True):
        ...
        with st.expander("Expand"):
            ...
            with col2:
                ...
                # --- Buy Points to Reduce Rate (moved here, below Refinance) ---
                buy_points = st.checkbox("Buy Points to Reduce Rate?", value=True, help="Pay points to lower mortgage rate.")
                points = 0.0
                ...
                # --- Buy Points block moved below Refinance ---
        ...
        with st.expander("Expand", expanded=False):
            st.markdown("Optionally model a refinance at a future date.")

            col1, col2 = st.columns(2)
            with col1:
                show_refinance = st.checkbox("Consider Refinance?", value=False, help="Toggle to include a refinance scenario.")
                refi_rate = st.number_input("Refinance Rate (%)", value=float(mortgage_rate), step=0.01, min_value=0.0, format="%.3f", help="Interest rate for refinanced loan.")
                refi_term_years = st.number_input("Refinance Term (Years)", value=int(loan_years), min_value=1, max_value=50, help="Duration of refinanced loan.")
                refi_start_date = st.date_input("Refinance Start Date", value=pd.to_datetime(f"{purchase_year+1}-01-01").date(), min_value=pd.to_datetime(f"{purchase_year}-01-01").date(), max_value=pd.to_datetime(f"{purchase_year + loan_years}-12-31").date(), help="Date refinance begins.")
            ...
            with col2:
                ...
                refi_payment_frequency = st.selectbox("Refi Payment Frequency", ["Monthly", "Biweekly"], index=0, help="Payment frequency for refinanced loan.")
                ...
                # Initialize refinance points variables
                refi_points = 0.0
                ...
                if refi_buy_points:
                    ...
                    with c1:
                        ...
                        refi_discount_per_point = st.number_input("Refi Rate Discount per Point (%)", value=0.25, step=0.01, min_value=0.0, help="Rate reduction per refinance point.")
                    with c2:
                        refi_points_cost_method = st.selectbox("Refi Points Cost Method", ["Add to Loan Balance", "Pay Upfront"], index=0, help="Finance or pay refinance points upfront.")
            ...
            if refi_mortgage_type == "Variable":
                ...
                default_refi_schedule = pd.DataFrame({
                    "Year": [1, 5, 10],
                    "Rate (%)": [refi_effective_rate, refi_effective_rate + 1.5, refi_effective_rate + 2.0]
                })
                refi_rate_schedule = st.data_editor(
                    default_refi_schedule,
                    column_config={
                        "Year": st.column_config.NumberColumn("Year", min_value=1, max_value=refi_term_years, step=1, help="Year the refi rate applies."),
        ...
        st.markdown("#### Homeownership Expenses")
        with st.expander("Expand", expanded=False):
            ...
            with st.container(border=False):
                col1, col2 = st.columns(2)
                with col1:
                    # replaced section_subtitle call "Recurring (Annual)")
                ...
                with col2:
                    # replaced section_subtitle call "One-time")
...
with col_rent:
    ...
    with st.container(border=True):
        ...
        with st.expander("Expand"):
            col1, col2 = st.columns(2)
            with col1:
                # replaced section_subtitle call "Recurring (Monthly/Annual)")
            ...
            with col2:
                # replaced section_subtitle call "One-time / Per-lease")
...
# Prepare baseline (without extra payments) annual summary immediately after it is created
...
thick_divider()
st.header("2. Mortgage Metrics")
# 3 columns w/timeline
...
def _payoff_date(df):
    try:
        mask = (df['Balance'] <= 0)
        if mask.any():
            return pd.to_datetime(df.loc[mask, 'Date']).min()
        else:
...
def _sum(df, col):
    try:
        return float(df[col].sum()) if (df is not None and col in df.columns) else 0.0
    except Exception:
        return 0.0

# Standardized DataFrame names (avoid ambiguous truth checks)
...
# Determine payments-per-year for monthly equivalent calc
main_ppy = globals().get('main_periods_per_year', 12) or 12

# --- ORIGINAL (NO REFI) METRICS ---
orig_loan_amount_eff = globals().get('display_loan_amount', globals().get('loan_amount', None))
orig_points_cost     = globals().get('points_cost', 0.0)
orig_points          = globals().get('points', None)
orig_discount_pt     = globals().get('discount_per_point', None)
orig_rate_eff        = globals().get('effective_mortgage_rate', globals().get('effective_rate', None))
orig_term_years      = globals().get('loan_years', None)

# Monthly payment from whichever DF exists (prefer no-refi path)
orig_payment_per_period = None
if df_no_refi is not None and len(df_no_refi) > 0:
    try:
        orig_payment_per_period = float(df_no_refi['Payment'].iloc[0])
    except Exception:
        orig_payment_per_period = None
if orig_payment_per_period is None and df_with_refi is not None and len(df_with_refi) > 0:
    try:
        orig_payment_per_period = float(df_with_refi['Payment'].iloc[0])
    except Exception:
        pass
orig_monthly_equiv = (orig_payment_per_period if main_ppy == 12 else (orig_payment_per_period or 0.0) * 26.0/12.0) if (orig_payment_per_period is not None) else None

# Totals (no refi path)
orig_total_interest = _sum(df_no_refi, 'Interest')
orig_total_pmi      = _sum(df_no_refi, 'PMI')
orig_total_principal = 0.0
if df_no_refi is not None:
    if 'Principal' in df_no_refi.columns:
        orig_total_principal += _sum(df_no_refi, 'Principal')
    if 'Extra Principal' in df_no_refi.columns:
        orig_total_principal += _sum(df_no_refi, 'Extra Principal')
orig_extra_principal_total = _sum(df_no_refi, 'Extra Principal')

# Start/Payoff dates (prefer no-refi, fallback to with-refi)
if df_no_refi is not None and len(df_no_refi) > 0:
    orig_start_date = _first_date(df_no_refi)
    orig_payoff_date = _payoff_date(df_no_refi)
elif df_with_refi is not None and len(df_with_refi) > 0:
    orig_start_date = _first_date(df_with_refi)
    orig_payoff_date = _payoff_date(df_with_refi)
else:
    orig_start_date = None
    orig_payoff_date = None

# --- REFI SNAPSHOT (AT REFI DATE) ---
refi_date = globals().get('refi_start_date', None)
refi_points_cost = globals().get('refi_points_cost', 0.0)
refi_points      = globals().get('refi_points', None)
refi_discount_pt = globals().get('refi_discount_per_point', None)
refi_costs       = globals().get('refi_costs', 0.0)
refi_balance_at_date = None
extra_to_refi = None
principal_to_refi = None
interest_to_refi = None
pmi_to_refi = None
if df_with_refi is not None and refi_date is not None:
    try:
        df_with_refi['Date'] = pd.to_datetime(df_with_refi['Date'])
    except Exception:
        pass
    try:
        idxs = None
        if 'Loan Type' in df_with_refi.columns:
            idxs = df_with_refi.index[df_with_refi['Loan Type'] == 'Refinance']
            idxs = list(idxs) if len(idxs) else None
        if not idxs:
            idxs = df_with_refi.index[df_with_refi['Date'] >= pd.to_datetime(refi_date)]
            idxs = list(idxs) if len(idxs) else None
        if idxs:
            i0 = idxs[0]
            if i0 > 0:
                refi_balance_at_date = float(df_with_refi.loc[i0-1, 'Balance'])
            else:
                refi_balance_at_date = float(df_with_refi.loc[i0, 'Balance'])
            sl = df_with_refi.iloc[:max(i0,1)]
            extra_to_refi     = _sum(sl, 'Extra Principal')
            principal_to_refi = _sum(sl, 'Principal')
            interest_to_refi  = _sum(sl, 'Interest')
            pmi_to_refi       = _sum(sl, 'PMI')
    except Exception:
        pass

# --- WITH-REFI PATH METRICS (AFTER REFI) ---
refi_rate_eff   = globals().get('refi_effective_rate', globals().get('refi_rate', None))
refi_term_years = globals().get('refi_term_years', None)
# Payment right after refi
refi_payment_after = None
if df_with_refi is not None:
    try:
        if 'Loan Type' in df_with_refi.columns:
            i1s = df_with_refi.index[df_with_refi['Loan Type'] == 'Refinance']
            if len(i1s):
                refi_payment_after = float(df_with_refi.loc[i1s[0], 'Payment'])
    except Exception:
        pass
# Monthly equivalent
refi_ppy = globals().get('refi_periods_per_year', 12) or 12
refi_monthly_equiv = (refi_payment_after if refi_ppy == 12 else (refi_payment_after or 0.0) * 26.0/12.0) if (refi_payment_after is not None) else None

# Totals on with-refi path
with_total_interest  = _sum(df_with_refi, 'Interest')
with_total_pmi       = _sum(df_with_refi, 'PMI')
with_total_principal = 0.0
if df_with_refi is not None:
    if 'Principal' in df_with_refi.columns:
        with_total_principal += _sum(df_with_refi, 'Principal')
    if 'Extra Principal' in df_with_refi.columns:
        with_total_principal += _sum(df_with_refi, 'Extra Principal')
with_extra_principal_total = _sum(df_with_refi, 'Extra Principal')
with_payoff_date = _payoff_date(df_with_refi) if (df_with_refi is not None and len(df_with_refi) > 0) else None

# Payoff difference (months): (with_refi payoff - no_refi payoff)
payoff_diff_months = None
try:
    if orig_payoff_date is not None and with_payoff_date is not None:
        payoff_diff_months = (with_payoff_date.to_period('M') - orig_payoff_date.to_period('M')).n
except Exception:
    payoff_diff_months = None

# Savings vs No-Refi: (Interest+PMI delta) - total refi costs
total_refi_cost_all = (refi_costs or 0.0) + (refi_points_cost or 0.0)
savings_vs_no_refi_financing = None
try:
    if (orig_total_interest is not None) and (with_total_interest is not None):
        orig_cost = (orig_total_interest or 0.0) + (orig_total_pmi or 0.0)
        with_cost = (with_total_interest or 0.0) + (with_total_pmi or 0.0)
        savings_vs_no_refi_financing = orig_cost - with_cost - total_refi_cost_all
except Exception:
    savings_vs_no_refi_financing = None

with st.expander("Expand"):
    c1, c2, c3 = st.columns(3)
    # Original (No Refi Path)
    with c1:
        st.subheader("Original (No Refi)")
        st.metric("Loan Amount (Effective)", _fmt_money0(orig_loan_amount_eff))
        st.metric("Points Purchased (Original)", f"{_fmt_money0(orig_points_cost)}" + (f"  •  {_fmt_pct((orig_discount_pt or 0.0))} / pt" if orig_discount_pt is not None else ""))
        st.metric("Term (Years)", _fmt_int(orig_term_years))
        st.metric("Effective Rate", _fmt_pct(orig_rate_eff))
        st.metric("Monthly Payment", _fmt_money0(orig_monthly_equiv))
        st.metric("Total Extra Principal (Original Term)", _fmt_money0(orig_extra_principal_total))
        st.metric("Total Principal Paid", _fmt_money0(orig_total_principal))
        st.metric("Total Interest Paid", _fmt_money0(orig_total_interest))
        st.metric("Total PMI Paid", _fmt_money0(orig_total_pmi))
        st.metric("Payoff Date (No Refi)", orig_payoff_date.strftime("%Y-%m") if isinstance(orig_payoff_date, pd.Timestamp) else "—")

    # Snapshot at Refi
    with c2:
        st.subheader("At Refi Date")
        st.metric("Refi Date", refi_date.strftime("%Y-%m-%d") if isinstance(refi_date, (pd.Timestamp,)) else (refi_date.strftime("%Y-%m-%d") if refi_date else "—"))
        st.metric("Remaining Balance", _fmt_money0(refi_balance_at_date))
        st.metric("Refi Costs (Closing)", _fmt_money0(refi_costs))
        st.metric("Points Purchased (Refi)", _fmt_money0(refi_points_cost) + (f"  •  {_fmt_pct((refi_discount_pt or 0.0))} / pt" if refi_discount_pt is not None else ""))
        st.metric("Extra Principal to Refi", _fmt_money0(extra_to_refi))
        st.metric("Principal Paid to Refi", _fmt_money0(principal_to_refi))
        st.metric("Interest Paid to Refi", _fmt_money0(interest_to_refi))
        st.metric("PMI Paid to Refi", _fmt_money0(pmi_to_refi))

    # With Refi Path
    with c3:
        st.subheader("With Refi Path")
        st.metric("Refi Effective Rate", _fmt_pct(refi_rate_eff))
        st.metric("Refi Term (Years)", _fmt_int(refi_term_years))
        st.metric("Monthly Payment (Post-Refi)", _fmt_money0(refi_monthly_equiv))
        st.metric("Total Extra Principal (Refi Path)", _fmt_money0(with_extra_principal_total))
        st.metric("Total Principal Paid (With Refi)", _fmt_money0(with_total_principal))
        st.metric("Total Interest Paid (With Refi)", _fmt_money0(with_total_interest))
        st.metric("Total PMI Paid (With Refi)", _fmt_money0(with_total_pmi))
        st.metric("Payoff Date (With Refi)", with_payoff_date.strftime("%Y-%m") if isinstance(with_payoff_date, pd.Timestamp) else "—")
        st.metric("Payoff Difference (Months)", (_fmt_int(payoff_diff_months) + " vs No Refi") if payoff_diff_months is not None else "—")
        st.metric("Savings vs No Refi (Int+PMI – Refi Cost)", _fmt_money0(savings_vs_no_refi_financing))

# Timeline chart (Start -> Refi -> Payoff)
try:
    start_dt = orig_start_date if isinstance(orig_start_date, pd.Timestamp) else (pd.to_datetime(orig_start_date) if orig_start_date else None)
    refi_dt  = refi_date if isinstance(refi_date, pd.Timestamp) else (pd.to_datetime(refi_date) if refi_date else None)
    payoff_no_refi = orig_payoff_date if isinstance(orig_payoff_date, pd.Timestamp) else (pd.to_datetime(orig_payoff_date) if orig_payoff_date else None)
    payoff_with_refi = with_payoff_date if isinstance(with_payoff_date, pd.Timestamp) else (pd.to_datetime(with_payoff_date) if with_payoff_date else None)

    xs, ys, labels, hover = [], [], [], []

    if start_dt is not None:
        xs.append(start_dt); ys.append(0); labels.append("Start")
        hover.append(f"Start<br>Loan: {_fmt_money0(orig_loan_amount_eff)}<br>Points: {_fmt_money0(orig_points_cost)}<br>Monthly: {_fmt_money0(orig_monthly_equiv)}")

    if refi_dt is not None:
        xs.append(refi_dt); ys.append(0); labels.append("Refi")
        hover.append(f"Refi<br>Balance: {_fmt_money0(refi_balance_at_date)}<br>Costs: {_fmt_money0(refi_costs)}<br>Points: {_fmt_money0(refi_points_cost)}<br>Extra to date: {_fmt_money0(extra_to_refi)}")

    if payoff_no_refi is not None:
        xs.append(payoff_no_refi); ys.append(0); labels.append("Payoff (No Refi)")
        hover.append(f"No Refi Payoff<br>Total Interest: {_fmt_money0(orig_total_interest)}<br>Total PMI: {_fmt_money0(orig_total_pmi)}")

    if payoff_with_refi is not None:
        xs.append(payoff_with_refi); ys.append(0); labels.append("Payoff (With Refi)")
        hover.append(f"With Refi Payoff<br>Total Interest: {_fmt_money0(with_total_interest)}<br>Total PMI: {_fmt_money0(with_total_pmi)}<br>Savings: {_fmt_money0(savings_vs_no_refi_financing)}")

    if xs:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=xs, y=ys, mode="markers+lines+text",
                                 text=labels, textposition="top center",
                                 hovertext=hover, hoverinfo="text"))
        fig.update_layout(height=260, margin=dict(l=10,r=10,t=20,b=10),
                          showlegend=False, xaxis_title="Timeline", yaxis=dict(visible=False))
...
if show_refinance and refi_start_date:
    st.subheader("Savings from Refinancing")
...
st.subheader("Savings by Paying Points (Timeline-Aware)")
...
# ============================================================
# Savings Scenarios (Refi / Points / Extra Payments) - Unified
# ============================================================
st.subheader("Savings Scenarios (Refi / Points / Extra Payments)")

tabs = st.tabs(["Refinance", "Buy Points", "Extra Payments"])

# ---- Common helpers ----
def _breakeven_row(df, cum_col, cost_col):
    mask = df[cum_col] >= df[cost_col]
    if mask.any():
        idx = mask.idxmax()
        return int(df.loc[idx, "Year"]), float(df.loc[idx, cum_col])
    return None, None

def _payoff_year_from_schedule(schedule_df):
    if "Balance" not in schedule_df.columns:
        return None
    try:
        done_idx = schedule_df.index[schedule_df["Balance"] <= 0]
        if len(done_idx) > 0:
            return int(schedule_df.loc[done_idx[0], "Year"])
        return int(schedule_df["Year"].iloc[-1])
    except Exception:
        return None

# Precompute payoff year from the main schedule (includes extra payments / refi / points effects already)
payoff_year = _payoff_year_from_schedule(schedule_with_extra_df)
...
with tabs[0]:
    if show_refinance and refi_start_date:
        ...
        _m = _aw.merge(
            _nr[["Year","Cum Interest","Cum PMI"]].rename(
                columns={"Cum Interest":"Cum Interest (No Refi)","Cum PMI":"Cum PMI (No Refi)"}
            ),
            on="Year", how="inner"
        )

        # Net savings vs original (allow negatives for honesty)
        _m["Interest Saved (Cum)"] = (_m["Cum Interest (No Refi)"] - _m["Cum Interest"])
        _m["PMI Saved (Cum)"] = (_m["Cum PMI (No Refi)"] - _m["Cum PMI"])
        _m["Total Saved (Cum)"] = _m["Interest Saved (Cum)"] + _m["PMI Saved (Cum)"]
        _m["Interest Saved (Year)"] = _m["Interest Saved (Cum)"].diff().fillna(_m["Interest Saved (Cum)"])
        _m["PMI Saved (Year)"] = _m["PMI Saved (Cum)"].diff().fillna(_m["PMI Saved (Cum)"])
        _m["Total Saved (Year)"] = _m["Interest Saved (Year)"] + _m["PMI Saved (Year)"]

        # Costs at refi date (treat points rolled into balance as upfront at refi date for breakeven)
        refi_upfront_cost = float(refi_costs) if 'refi_costs' in globals() else 0.0
        if 'refi_points_cost' in globals():
            if 'refi_points_cost_method' in globals() and refi_points_cost_method == "Pay Upfront":
                refi_upfront_cost += float(refi_points_cost)
            else:
                # If added to balance, still consider as upfront for cost-breakeven
                refi_upfront_cost += float(refi_points_cost)

        # Cumulative cost line from the refi year forward
        _m["Action Cost (Cum)"] = 0.0
        refi_year = refi_start_date.year
        _m.loc[_m["Year"] >= refi_year, "Action Cost (Cum)"] = refi_upfront_cost

        # --- Breakeven calculations (two types) ---
        # 1) Cost Breakeven: first year cumulative savings >= upfront costs
        _mask_cost = _m["Total Saved (Cum)"] >= _m["Action Cost (Cum)"]
        cost_be_year = int(_m.loc[_mask_cost, "Year"].iloc[0]) if _mask_cost.any() else None
        cost_be_val = float(_m.loc[_m["Year"] == cost_be_year, "Total Saved (Cum)"].iloc[0]) if cost_be_year is not None else None

        # 2) Crossover: first year AFTER cost breakeven where cumulative savings falls to <= 0
        cross_be_year = None
        cross_be_val = None
        if cost_be_year is not None:
            _post = _m[_m["Year"] >= cost_be_year]
            _mask_cross = _post["Total Saved (Cum)"] <= 0
            if _mask_cross.any():
                cross_be_year = int(_post.loc[_mask_cross, "Year"].iloc[0])
                cross_be_val = float(_post.loc[_post["Year"] == cross_be_year, "Total Saved (Cum)"].iloc[0])

        # Also compute payoff year if available
        def _payoff_year_from_schedule(schedule_df):
            if "Balance" not in schedule_df.columns:
                return None
            try:
                ...
                return None
        payoff_year = _payoff_year_from_schedule(schedule_with_extra_df) if 'schedule_with_extra_df' in globals() else None

        # ---- Summary line
        _summary_bits = []
        _summary_bits.append(f"Cost Breakeven: Year {cost_be_year}" if cost_be_year is not None else "Cost Breakeven: —")
        _summary_bits.append(f"Crossover: Year {cross_be_year}" if cross_be_year is not None else "Crossover: —")
        st.caption(" | ".join(_summary_bits))

        # ---- By Year chart (Net Impact, colored for +/-)
        fig_r_y = go.Figure()
        # Split positive and negative for color clarity
        pos_y = _m["Total Saved (Year)"].clip(lower=0)
        neg_y = _m["Total Saved (Year)"].clip(upper=0)
        fig_r_y.add_bar(x=_m["Year"], y=pos_y, name="Net Savings (Year)")
        fig_r_y.add_bar(x=_m["Year"], y=neg_y, name="Net Extra Cost (Year)")
        # Lines for components for transparency
        fig_r_y.add_scatter(x=_m["Year"], y=_m["Interest Saved (Year)"], name="Interest Component (Year)", mode="lines", visible="legendonly")
        if "PMI Saved (Year)" in _m:
            fig_r_y.add_scatter(x=_m["Year"], y=_m["PMI Saved (Year)"], name="PMI Component (Year)", mode="lines", visible="legendonly")
        # Vertical lines
        fig_r_y.add_vline(x=refi_year, line_dash="dash", line_width=2, annotation_text="Refi", annotation_position="top")
        if payoff_year: 
            fig_r_y.add_vline(x=payoff_year, line_dash="dash", line_width=2, annotation_text="Payoff", annotation_position="top")
        if cost_be_year is not None:
            fig_r_y.add_vline(x=cost_be_year, line_dash="dot", line_width=2, line_color="green", annotation_text="Cost Breakeven", annotation_position="top")
            fig_r_y.add_scatter(x=[cost_be_year], y=[_m.loc[_m["Year"]==cost_be_year, "Total Saved (Year)"].iloc[0]], mode="markers+text", marker=dict(size=10, color="green"), text=["Cost BE"], textposition="top center", showlegend=False)
        if cross_be_year is not None:
            fig_r_y.add_vline(x=cross_be_year, line_dash="dot", line_width=2, line_color="red", annotation_text="Crossover", annotation_position="top")
            fig_r_y.add_scatter(x=[cross_be_year], y=[_m.loc[_m["Year"]==cross_be_year, "Total Saved (Year)"].iloc[0]], mode="markers+text", marker=dict(size=10, color="red"), text=["Cross"], textposition="top center", showlegend=False)
        fig_r_y.update_layout(barmode="relative", yaxis_title="Annual Net Impact vs Original")
        style_plot(fig_r_y)
        st.plotly_chart(fig_r_y, use_container_width=True)

        # ---- Cumulative chart with breakeven markers
        fig_r_c = go.Figure()
        fig_r_c.add_scatter(x=_m["Year"], y=_m["Total Saved (Cum)"], mode="lines+markers", name="Total Saved (Cum)")
        fig_r_c.add_scatter(x=_m["Year"], y=_m["Action Cost (Cum)"], mode="lines", name="Action Cost (Cum)")
        fig_r_c.add_scatter(x=_m["Year"], y=_m["Cum Interest (No Refi)"], mode="lines", name="Cum Interest (No Refi)", visible="legendonly")
        # Vlines
        fig_r_c.add_vline(x=refi_year, line_dash="dash", line_width=2, annotation_text="Refi", annotation_position="top")
        if payoff_year: 
            fig_r_c.add_vline(x=payoff_year, line_dash="dash", line_width=2, annotation_text="Payoff", annotation_position="top")
        # Breakeven markers (fixed labels)
        if cost_be_year is not None:
            fig_r_c.add_vline(x=cost_be_year, line_dash="dot", line_width=2, line_color="green")
            fig_r_c.add_scatter(x=[cost_be_year], y=[cost_be_val], mode="markers+text", marker=dict(size=10, color="green"), text=["Cost Breakeven"], textposition="top center", name="Cost Breakeven")
        if cross_be_year is not None:
            fig_r_c.add_vline(x=cross_be_year, line_dash="dot", line_width=2, line_color="red")
            fig_r_c.add_scatter(x=[cross_be_year], y=[cross_be_val], mode="markers+text", marker=dict(size=10, color="red"), text=["Crossover"], textposition="top center", name="Crossover")

        fig_r_c.update_yaxes(title_text="Cumulative Net Savings vs Original")
        style_plot(fig_r_c)
        st.plotly_chart(fig_r_c, use_container_width=True)

        # Explanatory text
        st.markdown(
            "**How to read this:** "
            "Blue bars/lines = savings; red bars = extra cost vs original. "
            "**Cost Breakeven** (green) is when cumulative savings first exceed upfront refi costs. "
            "**Crossover** (red) is when keeping the refi long enough makes total savings fall back to zero or below."
        )

        # ---- Table with breakeven year columns and row highlights
        # Add columns with exact year labels (e.g., 'Year 7'), else blank
        _m["Cost Breakeven Year"] = np.where(_m["Year"] == cost_be_year, "Year " + _m["Year"].astype(str), "")
        _m["Crossover Year"] = np.where(_m["Year"] == cross_be_year, "Year " + _m["Year"].astype(str), "")

        def _style_breakeven_rows(df):
            styles = pd.DataFrame("", index=df.index, columns=df.columns)
            if cost_be_year is not None:
                styles.loc[df["Year"] == cost_be_year, :] = "background-color: rgba(0, 128, 0, 0.12);"  # light green
            if cross_be_year is not None:
                styles.loc[df["Year"] == cross_be_year, :] = "background-color: rgba(255, 0, 0, 0.12);"  # light red
            return styles

        with st.expander("View Data Table"):
            _cols = ["Year","Interest Saved (Year)","PMI Saved (Year)","Total Saved (Year)","Interest Saved (Cum)","PMI Saved (Cum)","Total Saved (Cum)","Action Cost (Cum)","Cost Breakeven Year","Crossover Year"]
            _disp = _m[_cols].copy().round(2)
            try:
                st.dataframe(_disp.style.apply(_style_breakeven_rows, axis=None), hide_index=True, use_container_width=True)
            except Exception:
                st.dataframe(_disp, hide_index=True, use_container_width=True)

        # Legend for table highlights
        st.caption("✅ **Green row** = Cost Breakeven   |   ❌ **Red row** = Crossover Point")
    else:
        st.info("Turn on **Refinance** and choose a start date to see refinance savings.")
# ---------- TAB: Buy Points ----------
with tabs[1]:
    if buy_points and points > 0:
        # Build schedules with and without points (respect ARM vs Fixed)
        def _ann_rate_to_period(annual_pct, ppy=12):
            try:
                r = float(annual_pct)/100.0
                return r/ppy
            except Exception:
                return None
        ...
        loan_amount_local = globals().get('loan_amount', 0.0)
        mortgage_type_local = globals().get('mortgage_type', 'Fixed')
        mortgage_rate_local = globals().get('mortgage_rate', 0.0)
        purchase_year_local = globals().get('purchase_year', 2025)
        extra_sch_local = expand_extra_payments(extra_payments, purchase_year_local, loan_years, "Monthly")

        # No points rate schedule
        no_points_rate_schedule = rate_schedule.copy()
        if mortgage_type_local == "Fixed":
            no_points_rate_schedule["Rate (%)"] = mortgage_rate_local
        else:
            no_points_rate_schedule["Rate (%)"] = no_points_rate_schedule["Rate (%)"] + (discount_per_point * points)

        # With points schedule (already in rate_schedule / effective_rate)
        with_points_rate_schedule = rate_schedule.copy()

        # Origination schedules
        _, _, orig_no_pts = amortization_schedule(
            principal=loan_amount_local + (closing_costs if closing_costs_method == "Add to Loan Balance" else 0) + (0 if points_cost_method == "Pay Upfront" else points_cost),
            years=loan_years,
            periods_per_year=12,
            start_date=f"{purchase_year_local}-01-01",
            extra_schedule=extra_sch_local,
            rate_schedule=no_points_rate_schedule,
            mortgage_type=mortgage_type_local,
            purchase_year=purchase_year_local,
            mortgage_rate=mortgage_rate_local,
            pmi_rate=pmi_rate,
            pmi_equity_threshold=pmi_equity_threshold,
            purchase_price=purchase_price
        )
        _, _, orig_with_pts = amortization_schedule(
            principal=loan_amount_local + (closing_costs if closing_costs_method == "Add to Loan Balance" else 0) + (0 if points_cost_method == "Pay Upfront" else points_cost),
            years=loan_years,
            periods_per_year=12,
            start_date=f"{purchase_year_local}-01-01",
            extra_schedule=extra_sch_local,
            rate_schedule=with_points_rate_schedule,
            mortgage_type=mortgage_type_local,
            purchase_year=purchase_year_local,
        ...
        # Refi schedules (if applicable)
        pts_refi_year = refi_start_date.year if (show_refinance and refi_start_date) else None
        if show_refinance and refi_start_date:
            # No points into refi
            _, _, refi_no_pts = amortization_schedule(
                principal=refi_effective_principal + (0 if refi_points_cost_method == "Pay Upfront" else refi_points_cost),
                years=refi_term_years,
                periods_per_year=12,
                start_date=f"{refi_start_date.year}-01-01",
                extra_schedule=extra_sch_local,
                rate_schedule=no_points_rate_schedule,
                mortgage_type=refi_mortgage_type,
                purchase_year=purchase_year_local,
                mortgage_rate=refi_effective_rate,
                pmi_rate=pmi_rate,
                pmi_equity_threshold=pmi_equity_threshold,
                purchase_price=purchase_price
            )
            # With points into refi
            _, _, refi_with_pts = amortization_schedule(
                principal=refi_effective_principal + (0 if refi_points_cost_method == "Pay Upfront" else refi_points_cost),
                years=refi_term_years,
                periods_per_year=12,
                start_date=f"{refi_start_date.year}-01-01",
                ...
                mortgage_type=refi_mortgage_type,
                purchase_year=purchase_year_local,
                mortgage_rate=refi_effective_rate - (discount_per_point * points),
                pmi_rate=pmi_rate,
                pmi_equity_threshold=pmi_equity_threshold,
                purchase_price=purchase_price
            )
            # Combine origination (until refi year - 1) + refi (from refi year)
            def _stitch(a, b, cut_year):
                a = a.copy(); b = b.copy()
                return pd.concat([a[a["Date"].dt.year < cut_year], b[b["Date"].dt.year >= cut_year]], ignore_index=True)

            no_pts_all = _stitch(orig_no_pts, refi_no_pts, pts_refi_year)
            with_pts_all = _stitch(orig_with_pts, refi_with_pts, pts_refi_year)
        else:
            no_pts_all = orig_no_pts.copy()
            with_pts_all = orig_with_pts.copy()

        # Annualize
        npa = no_pts_all.copy()
        npa["Year"] = npa["Date"].dt.year
        npa_annual = npa.groupby("Year", as_index=False)[["Interest","PMI"]].sum()
        wpa = with_pts_all.copy()
        wpa["Year"] = wpa["Date"].dt.year
        wpa_annual = wpa.groupby("Year", as_index=False)[["Interest","PMI"]].sum()

        # Savings calc
        pts = wpa_annual.merge(
            npa_annual.rename(columns={"Interest":"Interest (No Pts)","PMI":"PMI (No Pts)"}),
            on="Year", how="inner"
        )
        pts["Interest Saved (Year)"] = (pts["Interest (No Pts)"] - wpa_annual["Interest"]).clip(lower=0)
        pts["PMI Saved (Year)"] = (pts["PMI (No Pts)"] - wpa_annual["PMI"]).clip(lower=0)
        pts["Total Saved (Year)"] = pts["Interest Saved (Year)"] + pts["PMI Saved (Year)"]
        pts["Interest Saved (Cum)"] = pts["Interest Saved (Year)"].cumsum()
        pts["PMI Saved (Cum)"] = pts["PMI Saved (Year)"].cumsum()
        pts["Total Saved (Cum)"] = pts["Interest Saved (Cum)"] + pts["PMI Saved (Cum)"]

        # Action cost = points cost upfront at origination (or at refi too if you model refi points; here using origination points)
        action_cost_pts = float(points_cost) if 'points_cost' in globals() else 0.0
        pts["Action Cost (Cum)"] = 0.0
        first_year = int(pts["Year"].iloc[0]) if len(pts)>0 else None
        if first_year is not None:
            pts.loc[pts["Year"] >= first_year, "Action Cost (Cum)"] = action_cost_pts

        # Charts
        fig_p_y = go.Figure()
        fig_p_y.add_bar(x=pts["Year"], y=pts["Interest Saved (Year)"], name="Interest Saved (Year)")
        fig_p_y.add_bar(x=pts["Year"], y=pts["PMI Saved (Year)"], name="PMI Saved (Year)")
        fig_p_y.add_scatter(x=pts["Year"], y=pts["Action Cost (Cum)"], name="Action Cost (Cum)", mode="lines")
        if payoff_year: fig_p_y.add_vline(x=payoff_year, line_dash="dot", line_width=2, annotation_text="Payoff", annotation_position="top")
        if show_refinance and refi_start_date: fig_p_y.add_vline(x=pts_refi_year, line_dash="dash", line_width=2, annotation_text="Refi", annotation_position="top")
        style_plot(fig_p_y)
        st.plotly_chart(fig_p_y, use_container_width=True)

        fig_p_c = go.Figure()
        fig_p_c.add_scatter(x=pts["Year"], y=pts["Total Saved (Cum)"], mode="lines+markers", name="Total Saved (Cum)")
        fig_p_c.add_scatter(x=pts["Year"], y=pts["Action Cost (Cum)"], mode="lines", name="Action Cost (Cum)")
        if payoff_year: fig_p_c.add_vline(x=payoff_year, line_dash="dot", line_width=2, annotation_text="Payoff", annotation_position="top")
        if show_refinance and refi_start_date: fig_p_c.add_vline(x=pts_refi_year, line_dash="dash", line_width=2, annotation_text="Refi", annotation_position="top")
        be_year, be_val = _breakeven_row(pts, "Total Saved (Cum)", "Action Cost (Cum)")
        if be_year is not None:
            fig_p_c.add_scatter(x=[be_year], y=[be_val], mode="markers+text", text=["Breakeven"], textposition="top center", name="Breakeven")
        style_plot(fig_p_c)
        st.plotly_chart(fig_p_c, use_container_width=True)

        with st.expander("View Data Table"):
            st.dataframe(
                pts[["Year","Interest Saved (Year)","PMI Saved (Year)","Total Saved (Year)","Interest Saved (Cum)","PMI Saved (Cum)","Total Saved (Cum)","Action Cost (Cum)"]].round(2),
                hide_index=True
            )
    else:
        st.info("Enable **Buy Points** to see the savings vs. no points.")

# ---------- TAB: Extra Payments ----------
with tabs[2]:
    # Assumes annual_with_extra and annual_without_extra already built above
    try:
        df_ep = annual_with_extra.copy()
        base = annual_without_extra.copy()
        # Savings
        df_ep["Interest Saved (Cum)"] = (base["Cum Interest"] - df_ep["Cum Interest"]).fillna(0)
        df_ep["PMI Saved (Cum)"] = (base["Cum PMI"] - df_ep["Cum PMI"]).fillna(0)
        df_ep["Interest Saved (Year)"] = df_ep["Interest Saved (Cum)"].diff().fillna(df_ep["Interest Saved (Cum)"])
        df_ep["PMI Saved (Year)"] = df_ep["PMI Saved (Cum)"].diff().fillna(df_ep["PMI Saved (Cum)"])
        df_ep["Total Saved (Year)"] = df_ep["Interest Saved (Year)"] + df_ep["PMI Saved (Year)"]
        df_ep["Total Saved (Cum)"] = df_ep["Interest Saved (Cum)"] + df_ep["PMI Saved (Cum)"]
        # Action cost = cumulative extra principal payments
        # Build monthly extra schedule vs actual
        if "Extra Principal Payments" in monthly_with_extra_df.columns:
            ep_monthly = monthly_with_extra_df.copy()
            ep_monthly["Year"] = ep_monthly["Date"].dt.year
            extra_by_year = ep_monthly.groupby("Year", as_index=False)[["Extra Principal Payments"]].sum().rename(columns={"Extra Principal Payments":"Extra Paid (Year)"})
            extra_by_year["Action Cost (Cum)"] = extra_by_year["Extra Paid (Year)"].cumsum()
            df_ep = df_ep.merge(extra_by_year[["Year","Action Cost (Cum)"]], on="Year", how="left")
        else:
            df_ep["Action Cost (Cum)"] = 0.0

        # Charts
        fig_e_y = go.Figure()
        fig_e_y.add_bar(x=df_ep["Year"], y=df_ep["Interest Saved (Year)"], name="Interest Saved (Year)")
        fig_e_y.add_bar(x=df_ep["Year"], y=df_ep["PMI Saved (Year)"], name="PMI Saved (Year)")
        fig_e_y.add_scatter(x=df_ep["Year"], y=df_ep["Action Cost (Cum)"], name="Action Cost (Cum)", mode="lines")
        if payoff_year: fig_e_y.add_vline(x=payoff_year, line_dash="dot", line_width=2, annotation_text="Payoff", annotation_position="top")
        style_plot(fig_e_y)
        st.plotly_chart(fig_e_y, use_container_width=True)

        fig_e_c = go.Figure()
        fig_e_c.add_scatter(x=df_ep["Year"], y=df_ep["Total Saved (Cum)"], mode="lines+markers", name="Total Saved (Cum)")
        fig_e_c.add_scatter(x=df_ep["Year"], y=df_ep["Action Cost (Cum)"], mode="lines", name="Action Cost (Cum)")
        if payoff_year: fig_e_c.add_vline(x=payoff_year, line_dash="dot", line_width=2, annotation_text="Payoff", annotation_position="top")
        be_year, be_val = _breakeven_row(df_ep, "Total Saved (Cum)", "Action Cost (Cum)")
        if be_year is not None:
            fig_e_c.add_scatter(x=[be_year], y=[be_val], mode="markers+text", text=["Breakeven"], textposition="top center", name="Breakeven")
        style_plot(fig_e_c)
        st.plotly_chart(fig_e_c, use_container_width=True)

        with st.expander("View Data Table"):
            st.dataframe(
                df_ep[["Year","Interest Saved (Year)","PMI Saved (Year)","Total Saved (Year)","Interest Saved (Cum)","PMI Saved (Cum)","Total Saved (Cum)","Action Cost (Cum)"]].round(2),
                hide_index=True
            )
    except Exception as _e:
        st.warning(f"Extra payments savings could not be computed: {_e}")

st.subheader("Evaluation Year Selection")
...
with st.container(border=True):
    ...
    with col1:
        ...
        fig_buy_cost_types.add_trace(go.Bar(x=buy_cost_types['Year'], y=buy_cost_types['One-time'], name='One-time', marker_color='#6A1B9A'))
        fig_buy_cost_types.add_trace(go.Bar(x=buy_cost_types['Year'], y=buy_cost_types['Repeating'], name='Repeating', marker_color='#1E88E5'))
...
st.subheader("Net Asset Value (Assets - Costs) Over Time")
...
# =====================================================
# 5. Investment Simulation — Stochastic (t for brokerage, lognormal for housing)
...
section_header("5. Investment Simulation — Stochastic")
...
# =====================================================
# 6. Monte Carlo — Probabilistic Outcomes
# =====================================================

section_header("6. Monte Carlo — Probabilistic Outcomes", "")
